...
func (pr *Predicates) Render() app.UI {
...
func (pr *Predicates) OnInputChange(ctx app.Context, e app.Event) {
	pr.inputValue = ctx.JSSrc().Get("value").String()
	pr.Update()
}
func (pr *Predicates) GenerateSeed(ctx app.Context, e app.Event) {
	...
	pr.theLevel = seed % 10
	pr.question1 = pr.pred()
	pr.seedClicked = true
	pr.Update()
}

func (pr *Predicates) finishClicked(ctx app.Context, e app.Event) {
...
func (pr *Predicates) pred() Question {
